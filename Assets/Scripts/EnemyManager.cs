...
public class EnemyManager : MonoBehaviour
{
    ...
    [SerializeField] float targetTimer = 240;
    ...
    const float MAXIMUM_ENEMY_COUNT = 75;
    const int HILL_MIN_ENEMY_WAVE_COUNT = 10;
    ...
    public void OnLevelUpEnemies()
    {
        ...
        if (GameManager.instance.trapStats)
        {
        ...
        GameManager.instance.EnemyThreatIncrease(enemyLevel);
    }
    ...
    IEnumerator SpawnEnemyBatch()
    {
        ...
        while (allowEnemySpawns)
        {
            ...
            int enemyBatchEnemyCount = Random.Range(1, enemyLevel + 1);
            float batchCost = 0;
            float batchSpawnDelay = 1f;
            ...
            while (enemyPoints < batchCost || enemyStats.Count >= MAXIMUM_ENEMY_COUNT)
            {
                yield return new WaitForEndOfFrame();
            }

            enemyPoints -= batchCost;
    ...
    public void SpawnHillBatch()
    {
        // create the enemy batch we want to spawn.
        List<GameObject> enemyBatch = new List<GameObject>();
        //int enemyBatchEnemyCount = Random.Range(3, 7);
        int enemyBatchEnemyCount = Random.Range(HILL_MIN_ENEMY_WAVE_COUNT, HILL_MIN_ENEMY_WAVE_COUNT + enemyLevel * 2);
